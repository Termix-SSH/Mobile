...
import type { TunnelStatus, SSHHost, TunnelConnection } from "../../../../types";
...
import type { TunnelSessionProps } from "@/types";
...
export const TunnelManager = forwardRef<TunnelManagerHandle, TunnelSessionProps>(
  ({ hostConfig, isVisible, title = "Manage Tunnels", onClose }, ref) => {
    ...
    >({});
    const [loadingTunnels, setLoadingTunnels] = useState<Set<string>>(
      new Set()
    );
    ...
    const handleTunnelAction = async (
      ...
      const tunnel = hostConfig.tunnelConnections[tunnelIndex];
      const tunnelName = `${hostConfig.name || `${hostConfig.id}`}_${tunnel.sourcePort}_${tunnel.endpointHost}_${tunnel.endpointPort}`;
      ...
      try {
        if (action === "connect") {
          ...
          // Build the full host object from hostConfig
          const sourceHost: Partial<SSHHost> = {
            id: hostConfig.id,
            name: hostConfig.name,
            ip: "", // We don't have these in hostConfig
            port: 0,
            username: "",
            authType: "none",
            folder: "",
          ...
          // We need to fetch the full host data to get auth details
          const fullHost = allHosts.find((h) => h.id === hostConfig.id);
          if (!fullHost) {
          ...
          const tunnelConfig = {
            ...
            sourceSSHKey:
              fullHost.authType === "key" ? fullHost.key : undefined,
            sourceKeyPassword:
    ...
    return (
      ...
      >
        ...
        ) : !hostConfig.enableTunnel ||
          !hostConfig.tunnelConnections ||
          hostConfig.tunnelConnections.length === 0 ? (
          <View
            style={{
              flex: 1,
              alignItems: "center",
          ...
          >
            <View style={{ marginBottom: 12 }}>
              ...
              <Text style={{ color: "#9CA3AF", fontSize: 14, marginTop: 4 }}>
                {hostConfig.tunnelConnections.length} tunnel
                {hostConfig.tunnelConnections.length !== 1 ? "s" : ""} configured
                for {hostConfig.name}
              </Text>
            </View>
            ...
            >
              {hostConfig.tunnelConnections.map((tunnel, idx) => {
                const tunnelName = `${hostConfig.name || `${hostConfig.id}`}_${tunnel.sourcePort}_${tunnel.endpointHost}_${tunnel.endpointPort}`;
